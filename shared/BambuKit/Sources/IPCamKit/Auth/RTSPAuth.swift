...
struct RTSPAuthenticator: Sendable {
  ...
  struct DigestState: Sendable {
    ...
    /// Whether the server's challenge named an algorithm. RFC 7616 makes the
    /// parameter optional (absent means MD5), and LIVE555-based servers — Bambu
    /// Lab printers among them — reject an otherwise-valid Digest response that
    /// carries an unsolicited `algorithm=MD5`, so echo it back only when asked.
    var algorithmSpecified: Bool = false
    var nc: UInt32 = 0
  }
  ...
  private func parseDigestChallenge(_ params: String) -> DigestState {
    ...
    return DigestState(
      ...
      opaque: opaque, algorithm: algorithm, algorithmSpecified: algorithmSpecified)
  }
