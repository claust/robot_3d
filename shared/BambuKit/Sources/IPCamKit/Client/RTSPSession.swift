...
public final class RTSPClientSession: Sendable {
  ...
  public convenience init(
    ...
    self.init(
      ...
      userAgent: userAgent, tls: tls, onDiagnostic: onDiagnostic, keepaliveInterval: nil)
  }
...
actor SessionState {
  ...
  func start(
    ...
    // RTSPS defaults to 322 (RFC 7826 §4.2), plain RTSP to 554. An rtsps://
    // URL implies TLS even when the caller passed no options.
    let isSecure = (urlComponents.scheme ?? "").lowercased() == "rtsps"
    let tlsOptions = tls ?? (isSecure ? TLSOptions() : nil)
    let rawPort = urlComponents.port ?? (isSecure ? 322 : 554)
    ...
    try await conn.connect(host: host, port: port, tls: tlsOptions)
    connection = conn
