...
actor RTSPTransportConnection {
  ...
  /// Connect to an RTSP server. Pass `tls` for an `rtsps://` endpoint.
  func connect(host: String, port: UInt16, tls: TLSOptions? = nil) async throws {
    let nwHost = NWEndpoint.Host(host)
    guard let nwPort = NWEndpoint.Port(rawValue: port) else {
      throw RTSPError.connectionFailed("Invalid port \(port)")
    }
    // The TLS handshake happens before .ready, so the existing connect timeout
    // and .waiting grace window cover it unchanged.
    let parameters = tls?.parameters(host: host, queue: queue) ?? .tcp
